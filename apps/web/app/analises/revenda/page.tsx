...
export default function AnalisesRevendaPage() {
  const { data: session } = useSession();
  const token = session?.accessToken;
  const queryClient = useQueryClient();

  const analisesQuery = useQuery({
    queryKey: ['analises', 'revenda'],
    queryFn: async () => {
      if (!token) throw new Error('Sessão inválida');
      return apiFetch<AnaliseRow[]>('/analises/revenda', { token });
    },
  ...
  const createAnalise = useMutation({
    mutationFn: async (payload: AnaliseInput) => {
      ...
      const custoBase =
        ...
        payload.comissao +
        payload.rateioFixas;
      const { precoSugerido } = await apiFetch<{ precoSugerido: number }>(`/precificacao/revenda/sugerir-preco`, {
        method: 'POST',
        body: {
          custo: custoBase,
      ...
      return apiFetch<AnaliseRow>('/analises/revenda', {
        method: 'POST',
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['analises', 'revenda'] });
    },
  });
  ...
  return (
    <section className="space-y-6">
      <header>
        <h2 className="text-2xl font-semibold">Análises - Revenda</h2>
        <p className="text-sm text-gray-600">Traga insights financeiros, preço sugerido e narrativa executiva.</p>
      </header>
        ...
        aiActions={[
          {
            ...
            onInvoke: async (values) => {
              const margemPerc = values.margemContrib / (values.precoVenda || 1) * 100;
              return `Produto ${values.produtoId} entrega margem de contribuição ${margemPerc.toFixed(
                1,
              )}% com lucro projetado de ${currency.format(values.margemLucro)}.`;
            },
          },
        ]}
      />
