...
export default function SimulacoesRevendaPage() {
  const { data: session } = useSession();
  const token = session?.accessToken;
  const queryClient = useQueryClient();

  const simulacoesQuery = useQuery({
    queryKey: ['simulacoes', 'revenda'],
    queryFn: async () => {
      if (!token) throw new Error('Sessão inválida');
      return apiFetch<SimulacaoRow[]>('/simulacoes/revenda', { token });
    },
  ...
  const createSimulacao = useMutation({
    mutationFn: async (payload: SimulacaoInput) => {
      if (!token) throw new Error('Sessão inválida');
      return apiFetch<SimulacaoRow>('/simulacoes/revenda', {
        method: 'POST',
        body: payload,
        token,
      });
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['simulacoes', 'revenda'] });
    },
  });
  ...
  return (
    <section className="space-y-6">
      <header>
        <h2 className="text-2xl font-semibold">Simulações - Revenda</h2>
        <p className="text-sm text-gray-600">Explore cenários de vendas diretas, mix e rateios de despesas fixas.</p>
      </header>
        ...
        aiActions={[
          {
            ...
            onInvoke: async (values) => {
              ...
              return `Cenário base projeta margem de ${margemPerc.toFixed(1)}% e faturamento ${currency.format(
                values.faturamentoPrevisto,
              )}. Considere mix ${values.percentualMix.toFixed(1)}% digital.`;
            },
          },
        ]}
      />
